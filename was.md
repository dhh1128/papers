...
* On Jan 1, Alice creates a key, K.
* A month later, on Feb 1, Malfoy steals Alice's key.
* A week later, on Feb 8, Alice becomes suspicious and revokes her key.
* At any time on or after Feb 1, Malfoy uses K to sign something in Alice's name, creating fake evidence of her intent during the time between Jan 1 and Feb 8.

![retrograde attack](assets/retrograde-attack.png)

A naïve analysis would assume that Alice is only vulnerable to Malfoy's mischief between Feb 1 and Feb 8. This is time period A in the diagram. Further, they would assume that any signed events that Malfoy creates for Alice during that time period must reference a timestamp between Feb 1 and Feb 8. This is time period B in the diagram.

Unfortunately, Alice's exposure in this situation is much worse than the comfortable assumption. Malfoy can easily backdate a fraudulent affidavit, mortgage, or business document to any point in time when K was known to be valid. This is time period C in the diagram &mdash; the true window for timestamps that an attacker can use.

It gets even worse. Nothing magic happens on Feb 8 that shuts down such retrograde attacks. Sure, Alice's key is revoked &mdash; but revocation only cancels future use, not past use. And Malfoy can keep generating new but backdated evidence (something that looks like "past use") *forever*. This is time period D in the diagram, and as the dotted lines at the right show, it has no end point. The vulnerability is perpetual.
...
Some business processes generate a lot of contextual clues like this. However, the clues are not always obvious, human attention and wisdom are both flawed, and clues are an iffy solution that doesn't always apply.
...
Another problem with this is that Alice can't force every stakeholder to honor her wishes; damage is already occurring. If Christie's is auctioning a Basquiat painting, and is about to receive Alice's signed bid, is it fair to the other bidders, the seller, or the auction house to let Alice off the hook, undoing thirty seconds of dramatic activity by many different parties? What if another bidder will give up when they heard Alice's bid? Or what if they *won't* give up, and Alice's bid would prolong a bidding war that Christie's loves? Maybe altering the outcome of the auction, not actually buying the painting, was actually Malfoy's goal all along, but how do we know *which* alterations to cancel?
...
Think for a minute about the consequences for issuers and holders of VCs that use DIDs as identifiers. If an issuer has a key compromise, they can rotate keys &mdash; but doing so *invalidates all the evidence they have ever created*. This creates massive practical difficulties, because reissuing evidence may require new interactions with holders, and holders might not be available. What would you think of a driver's license bureau whose recover-from-breach plan required all current license holders to come back to the office?
...
If obvious mitigations for retrograde attacks aren't satisfying, what would a real solution look like? The formula is actually simple:
...
In reaction to the formula above, some might say, "Sounds like blockchain." And they would be partly right &mdash; blockchain *is* one way to do it. (Of course, this assumes that blockchain is applied correctly to the problem. If signing events happen without some provable link back to the blockchain, all bets are off. Maybe that's why many blockchain-based DID methods don't support `versionId` or `versionTime`...)
...
KERI's implementation of this requirment uses KELs (key event logs) and TELs (transaction event logs). It anchors (records the hashes of) signed TEL events in the KEL to make sequence unambiguous. The mechanism is lightweight and elegant, and I recommend it.
...
I am not claiming that ephemeral evidence should be eliminated. It's lightweight to create and to verify, and that might matter a lot. Login that doesn't require an audit trail is a ubiquitous use case in digital identity; can be solved nicely with ephemeral evidence. However, it is a seriously bad idea to identify issuers and holders of credentials in a way that can only be analyzed at the current moment. And many non-VC use cases for raw digital signatures require protection from retrograde attacks. Malfoy knows this. To frustrate him, we really need anchored signatures.
