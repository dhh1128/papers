...
In Europe, the clock has become a calendar. The NIS Cooperation Group's coordinated roadmap sets dated milestones — national roadmaps by the end of 2026, high-risk use cases migrated by the end of 2030, medium-risk by 2035 — and states that for high-risk use cases, quantum-vulnerable public-key mechanisms shall not be used stand-alone after the end of 2030. [13] The catalog of cryptography that EU certification rules point to is moving in step: its draft third version, in public review through July 2026, demotes every classical signature scheme it used to recommend to a time-limited *admissible* class, leaving only post-quantum schemes recommended. [14, 15]

Navigating this ambitious transition requires defending against a two-front war. The first front is the HNDL attack on confidentiality, which attempts to crack encrypted data in transit and at rest. [16] The second is a "surprise quantum attack" on control and authenticity. In this scenario, an adversary with a quantum computer could instantly break the signing keys that govern digital identities and assets, allowing them to forge signatures and seize control. [17, 18] Any robust solution must address both threats.
...
An Autonomic Identifier (AID) is a persistent, self-certifying identifier that serves as a control mechanism for an entity engaged in cryptographic interactions. [19]
...
The most direct threat a quantum computer poses to a digital identity system is a "surprise attack" on control. This attack targets the asymmetric signing keys, such as ECDSA or Ed25519, that are used to prove authority over an identifier. [17, 18] With a sufficiently powerful quantum computer, an adversary could use Shor's algorithm to take a public signing key — which is, by design, visible to the world — and derive the corresponding private key in a matter of hours or minutes. [20] With this stolen key, the attacker could forge signatures, issue fraudulent credentials, and hijack the identity.
...
KERI's pre-rotation works differently. When an identity is first created, or at the moment of any subsequent key rotation, the controller makes a binding commitment to the next set of rotation keys. However, they do not publish the public keys of this heir apparent. Instead, they publish only a cryptographic digest — a hash — of those public keys. [19]

This digest acts as an unbreakable royal seal on a secret decree of succession. It provides a verifiable, forward cryptographic commitment to the identity of the next in line, but it does so without revealing who that heir is. This is the critical step. The security of this commitment rests on the one-way nature of the hash function. Modern cryptographic hashes like SHA-256 are considered post-quantum safe; while Grover's algorithm provides a theoretical speedup for finding preimages, simply using a 256-bit hash provides an effective 128 bits of post-quantum security, a level of strength that remains far beyond the reach of any feasible attack. [17, 18] European certification guidance draws the same line between mature and novel primitives: the new lattice-based schemes must be combined with a classically secure mechanism before they carry security on their own, while hash-based mechanisms may be used standalone. [14] Its draft revision does ask for 384-bit hash outputs where quantum security is sought, on the ground that a 256-bit output's quantum security level could arguably drop below 128 bits — a parameter choice, not a change of primitive, and one KERI's agility accommodates by design. [15] The line of succession is therefore sealed with quantum-resistant cryptography from the very beginning.
...
However, the legitimate controller, who is expected to monitor their own identity's history via a local "watcher," will detect this forgery. [19] Upon detection, the controller can initiate a planned and orderly succession. They publish a formal rotation event. This event does two things:
...
The most straightforward way to defeat an HNDL attack on a key exchange is to avoid a public, asymmetric key exchange altogether. Two parties can establish a shared secret — a "salt" — through an out-of-band (OOB) channel, such as during an in-person meeting or by exchanging a physical token. [16] This shared secret can then be used with a deterministic key derivation function to generate any number of symmetric encryption keys for secure communication. Because no quantum-vulnerable public-key cryptography is used to establish the secret, there is nothing for an HNDL adversary to capture and later decrypt. This approach is particularly practical because many highly secure systems already require some form of OOB step to bootstrap trust and prevent man-in-the-middle attacks. [21, 22]
...
This hybrid model is not merely theoretical; it is already being deployed in the wild as the most responsible path forward. OpenSSH, the software that secures countless remote connections across the internet, began deploying a hybrid PQC key exchange by default in version 9.0, released in April 2022. [23] IETF has formally documented the approach [24], and GitHub recently adopted it as well. [25] This implementation combines Streamlined NTRU Prime, a PQC algorithm, with the classical X25519 algorithm. The fact that foundational pieces of internet infrastructure has adopted this model is a powerful validation of its security and practicality. European regulators have landed in the same place: the EU's agreed-mechanisms catalog directs that post-quantum cryptography be rolled out in hybrid mode alongside classically secure asymmetric cryptography, with every component of the hybrid required to break before the hybrid does. [14]
...
The EU has just shown what this looks like at the legal layer. Its 2026 amendment to the digital identity wallet regulations obliges providers to use only the cryptographic mechanisms named in Annex Ia — and Annex Ia names no mechanisms. It is a bare URL, undated and unversioned, pointing at a document ENISA maintains, while the same regulation pins an IETF draft to an exact revision. [26] A regulator can now revise its cryptographic requirements by editing a file; whether the systems underneath can follow is an architectural question.
...
[13] NIS Cooperation Group. 2025. A Coordinated Implementation Roadmap for the Transition to Post-Quantum Cryptography, Part 1, Version 1.1. European Commission. https://ec.europa.eu/newsroom/dae/redirection/document/117507

[14] European Cybersecurity Certification Group, Sub-group on Cryptography. 2025. Agreed Cryptographic Mechanisms, Version 2.0. ENISA. https://certification.enisa.europa.eu/publications/eucc-guidelines-cryptography_en

[15] European Cybersecurity Certification Group, Sub-group on Cryptography. 2026. Agreed Cryptographic Mechanisms, Version 3 — Draft for Public Review. ENISA. https://certification.enisa.europa.eu/publications/eucc-guidelines-cryptography_en

[16] Entrust. 2023. Harvest Now, Decrypt Later – Fact or Fiction? https://www.entrust.com/blog/2023/11/harvest-now-decrypt-later-fact-or-fiction

[17] Bernstein, D., and Lange, T. 2017. Post-quantum cryptography: Dealing with the fallout of physics success. IACR Cryptology ePrint Archive 2017/314. https://eprint.iacr.org/2017/314
...
[19] Smith, S. 2024. Key Event Receipt Infrastructure (KERI): Specification. Draft v0.9. Trust Over IP Foundation. https://trustoverip.github.io/kswg-keri-specification/

[20] Department of Homeland Security (DHS). 2025. Post-Quantum Cryptography Resources. https://www.dhs.gov/quantum

[21] National Institute of Standards and Technology (NIST). 2017. Digital Identity Guidelines: Authentication and Lifecycle (NIST Special Publication 800-63B). https://pages.nist.gov/800-63-3/sp800-63b.html

[22] Sethi, M., Sarikaya, B., and Garcia-Carrillo, D. 2025. Terminology and processes for initial security setup of IoT devices. Internet Research Task Force (IRTF). https://datatracker.ietf.org/doc/draft-irtf-t2trg-security-setup-iot-devices/05/

[23] OpenSSH Project. 2022. Release 9.0 Notes. https://www.openssh.com/txt/release-9.0

[24] Friedl, M., Mojzis, J., and Josefsson, S. 2025. Secure Shell (SSH) Key Exchange Method Using Hybrid Streamlined NTRU Prime sntrup761 and X25519 with SHA-512: sntrup761x25519-sha512. Internet Engineering Task Force. https://datatracker.ietf.org/doc/html/draft-ietf-sshm-ntruprime-ssh-06

[25] Carlson, B., and Blau, T. 2025. Post-quantum security for SSH access on GitHub. https://t.ly/9VdOl

[26] European Commission. 2026. Commission Implementing Regulation (EU) 2026/1731, Article 5a and Annex Ia. https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32026R1731
