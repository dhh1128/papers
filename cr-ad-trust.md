...
The core flaw lies in the separation of identifier from key. In the X.509 model, an identifier — a domain name, an email address, an organization name — is a record in a database. The key is a separate artifact. The binding between them is an assertion: a digital certificate signed by a third party. This places the root of trust outside the identity itself. We trust that a key controls an identifier not because of anything inherent in the identifier, but because we trust the administrator who signed it [1].
...
| Quantum posture | Migration required across all CAs, logs, certificates, and TLS stacks | Architectural: pre-rotation hides next key behind a hash; crypto-agility allows algorithm upgrades without changing the identifier |
...
| Deployment maturity | Globally deployed; quantum migration pending | Early standardization and production activity (ISO 17442-3, GSMA Open Verifiable Calling); no migration debt |
...
**Recovery rotation and formal ambiguity.** KERI's key recovery mechanism introduces a specific formal complexity. Under recovery rotation semantics, correct witnesses may sign both an attacker's fraudulent event and the legitimate owner's recovery event — because KERI prioritizes the ability to reclaim a compromised identity over strict uniqueness of history at every moment. The consequence is that during a recovery scenario, validators may see multiple mathematically valid KELs for the same identifier, and no cryptographic mechanism alone can determine which is legitimate [27]. Validators must use external evidence — observer records, timestamps, governance context — to figure out which fork represents the real controller's recovery. This is genuine operational friction in a crisis. It is also a deliberate tradeoff: the alternative would be an identity you cannot recover from compromise without involving a trusted third party — which brings back exactly the administrative dependency KERI was designed to escape. The tradeoff buys real-world recoverability at the cost of formal elegance in edge cases.

**The watcher burden.** KERI's most significant current limitation is the immaturity of its watcher infrastructure. KERI's security depends on an ambient network of watchers that actively poll witnesses, compare log states, and propagate duplicity evidence. Watchers cost money and effort to run. In early deployments, coverage may be sparse and polling inconsistent. Under these conditions, duplicity could persist longer than is safe for high-stakes applications. One natural response is to rely on a centralized archival service — a "super watcher" — that provides broad coverage. This works, but it reintroduces a centralized trust dependency that KERI was designed to avoid.
...
KERI's position is structurally different. Pre-rotation protects the next controlling key behind a hash from the moment of inception. Early adopters are building on post-quantum-capable foundations today. When signing algorithms need upgrading, KERI controllers can do so without changing their identifier, without re-issuing credentials to relying parties, and without coordinating with any CA or root program. The upgrade is an event in the KEL. Not a global infrastructure project.
...
The window in which that choice remains open is, according to Google's own engineers, approximately three years.
